...
% method 2 - height range point selection
% for cube wireframe - set 0 to 0.5 with 0.1 neighbourhood
% for armadillo - range is 0 to 2.5 with 0.1 neighbourhood

%  -15 to -14
z_min = 0; z_max = 1;
%z_min = -14.3; z_max = -13.5; %trefoil
...
saved_shapes = {};   % Cell array to store intermediate vertex positions
%save_iters = round(linspace(2, max_iter-1, 3));  % Pick 3 intermediate iterations to save
save_iters = [6, 12, 20];
saved_count = 0;

% video
record_video = false;  % Set to false to disable video

if record_video
    fig = figure('Position', [100 100 1280 960]);  % Large figure for high-res
    v = VideoWriter('shape_optimization_progress_cube.mp4', 'MPEG-4');
    v.Quality = 100;
    v.FrameRate = 10;
    open(v);
end
